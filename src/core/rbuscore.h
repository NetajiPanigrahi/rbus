 ...
 * a matching event is received. */
rbusCoreError_t rbuscore_subscribeToEventTimeout(const char * object_name,  const char * event_name, rbus_event_callback_t callback, const rbusMessage payload, void * user_data, int* providerError, int timeout_ms, bool publishOnSubscribe, rbusMessage *response, bool rawData);
...
rbusCoreError_t rbuscore_sendData(const void* data, uint32_t dataLength, const char * topic);
